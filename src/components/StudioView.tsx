...
export function StudioView({ scene }: { scene: SceneModule }) {
  ...
  const reviewWeakArea = useCallback(
    (challengeId: string) => {
      const mastery = summarizeMastery(scene);
      const adapted =
        challenges.find((c) => c.id === challengeId) ??
        (mastery ? buildAdaptedChallenge(scene.id, mastery.weakest) : null) ??
        sceneChallenges[0];
      if (adapted) startChallenge(adapted);
    },
    [scene, sceneChallenges, startChallenge],
  );
        ...
        <section className="panel relative order-1 min-h-[45vh] overflow-hidden lg:order-2">
          ...
          {mode === "challenge" && challenge ? (
            <ChallengeBar
              ...
              onFocusHotspot={(id) => controller.dispatch({ type: "focus", hotspotId: id })}
              onNarrow={(ids) => controller.dispatch({ type: "fadeOthers", hotspotIds: ids })}
          ...
          <ul className="space-y-1">
            {sceneChallenges.map((c) => (
              <li key={c.id}>
                ...
                >
                  {c.title} · {c.steps.length} step{c.steps.length > 1 ? "s" : ""}
                </button>
              </li>
            ))}
          </ul>
