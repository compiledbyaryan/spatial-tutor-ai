...
export function MasteryPanel({ scene, profile, onReviewWeakArea, onPracticeAgain }: Props) {
  ...
  return (
    <div className="panel p-4">
      <div className="flex items-center justify-between"><p className="label-mono text-primary">Mastery</p><span className="label-mono">{summary.totalAttempts} scored attempts</span></div>
      ...
      <div className="mt-4 flex flex-col gap-2">
        {summary.recommended ? <button onClick={() => onReviewWeakArea(summary.recommended!)} className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90"><Award className="h-4 w-4" /> Review weak area: {AXIS_LABEL[summary.weakest].toLowerCase()}<ArrowRight className="h-4 w-4" /></button> : null}
